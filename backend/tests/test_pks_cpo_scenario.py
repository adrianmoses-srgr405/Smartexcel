...
def test_pks_cpo_queries():
    ...
    res1 = client.post("/api/v1/query/analyze", json={"dataset_id": ds_id, "query": q1})
    ...
    gt = ans1['calculation_summary'].get('grand_total') or 0
    print(f"     Rows: {len(ans1['table_rows'])}, Grand Total CPO: {gt:,} Kg")

    # 2. Test Rendemen Average: "Berapa rata-rata rendemen CPO per asal kebun?"
    q2 = "Berapa rata-rata rendemen CPO per asal kebun?"
    res2 = client.post("/api/v1/query/analyze", json={"dataset_id": ds_id, "query": q2})
    ...
    res3 = client.post("/api/v1/query/analyze", json={"dataset_id": ds_id, "query": q3})
    ...
    res4 = client.post("/api/v1/query/analyze", json={"dataset_id": ds_id, "query": q4})
